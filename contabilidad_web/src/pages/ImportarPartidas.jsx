...
export default function ImportarPartidas() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/v1/partidas/importar-validar?empresa_id=${localStorage.getItem('empresa_activa')}&anio=${localStorage.getItem('anio_activo')}`, formData, {
        headers: {
      ...
      setStep(3);
        ...
        if (err.response?.status === 400 && err.response?.data?.detail?.errores) {
          setResults({ errores: err.response.data.detail.errores });
          setError('');
        } else {
  ...
  const ModalErrores = () => {
    if (!results || !results.errores || results.errores.length === 0) return null;
    return (
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[90vh]">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-rose-50">
            <h3 className="text-xl font-bold text-rose-700 flex items-center gap-2">
              <AlertTriangle className="w-6 h-6" /> Incongruencias Encontradas
            </h3>
            <button onClick={() => { setResults(null); setStep(1); setFile(null); setMapping({}); }} className="text-slate-400 hover:text-slate-600">
              Cerrar
            </button>
          </div>
          <div className="p-6 overflow-y-auto flex-1">
            <p className="text-slate-600 mb-4">
              La importacin ha sido detenida. Por favor, corrige los siguientes errores en tu archivo Excel/CSV y vuelve a intentarlo:
            </p>
            <div className="space-y-3">
              {results.errores.map((err, idx) => (
                <div key={idx} className="bg-rose-50/50 border border-rose-100 rounded-lg p-3 flex items-start gap-3 text-sm">
                  <div className="bg-rose-100 text-rose-600 rounded-md px-2 py-1 font-bold whitespace-nowrap">
                    P: {err.partida}
                  </div>
          ...
          <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end">
            <button onClick={() => { setResults(null); setStep(1); setFile(null); setMapping({}); }} className="px-6 py-2 bg-slate-800 text-white rounded-lg font-bold hover:bg-slate-900">
              Entendido
            </button>
          </div>
        </div>
  ...
  return (
      ...
      {results && results.errores && <ModalErrores />}
    </div>
  );
}
